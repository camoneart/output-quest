...
const LegalLink = () => {
  ...
  if (!isLoaded || isZennLinked === null) {
    return (
      <div className={`${styles["legal-link-box"]}`}>
        <span className={`${styles["legal-link-text"]}`}>読み込み中...</span>
      </div>
    );
  }
    ...
    <div className={`${styles["legal-link-box"]}`}>
      ...
      >
        <Image
          ...
          width={20}
          height={20}
          className={styles["legal-link-icon"]}
        />
        <span className={styles["legal-link-text"]}>{linkText}</span>
      </Link>
    </div>
  );
};
